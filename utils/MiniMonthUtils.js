...
class CalendarUtils {
          ...
          m(
            ".cell",
            {
              ...
              onclick: () => {
                if (cellInfo.isCurrentMonth) {
                  setCalendarDate(
                    new Date(currentYear, currentMonth, cellInfo.content)
                  );
                }
              },
            },
            [
              m("p", [
                ...
                cellInfo.eventPoints.length > 0
                  ? m(
                      ".event-points",
                      cellInfo.eventPoints.map((point, index) =>
                        m("span.point", { key: index }, point)
                      )
                    )
                  : null,
              ]),
  ...
  static generateCellContent(params) {
    ...
    if (isCurrentMonth) {
      ...
      isSelected =
        dayCounter === calendar.date.getDate() &&
        currentMonth === calendar.date.getMonth() &&
        currentYear === calendar.date.getFullYear();

      const eventCount = this.getEventsCountForDay(
        new Date(currentYear, currentMonth, dayCounter),
        events
      );
